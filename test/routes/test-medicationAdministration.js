...
var linkProperty = 'prescription.reference';
...
describe(testTitle, function () {
    ...
    var linkedResourceSets = [samples.linkedSet0(), samples.linkedSet1()];
    var moments = {
        start: moment()
    };
    ...
    var prescriptionlessSample = (function () {
        var clone = _.cloneDeep(resourceSets[0][0]);
        delete clone.prescription;
        return clone;
    })();
    it('fail to create resource 0 for patient 0 with prescription missing', fn(r, r.createNegative, prescriptionlessSample));
    it('fail to create resource 0 for patient 0 with prescription invalid', fn(r, r.createNegativeStatus, [resourceSets[0][0], 404]));
    ...
    it('assign patient refs to all prescription resources', function () {
        common.putPatientRefs(linkedResourceSets, patientSamples, patientProperty);
    });

    _.range(2).forEach(function (i) {
        _.range(linkedResourceSets[i].length).forEach(function (j) {
            var title = util.format('create prescription resource %s for patient %s', j, i);
            it(title, fn(pres, pres.create, [linkedResourceSets[i][j], moments]));
        }, this);
    }, this);

    it('assign prescription refs to all resources', function () {
        common.putPrescriptionRefs(resourceSets, linkedResourceSets, linkProperty);
    });
...
describe(testTitle + ' search by page', function () {
    ...
    var linkedResourceSets = common.multiplySampleSets(4, [samples.linkedSet0(), samples.linkedSet1()]);
    var moments = {
        start: moment()
    };
    ...
    it('search prescription by page initial call', fn(pres, pres.searchByPageInitial, [n, {}, "initial"]));

    describe('prescription initial call links', function () {
        it('first', fn(pres, pres.searchByPage, [n, {}, 'initial', 'first', 0, null]));
        it('last', fn(pres, pres.searchByPage, [n, {}, 'initial', 'last', lastPage, null]));
        it('self', fn(pres, pres.searchByPage, [n, {}, 'initial', 'self', 0, 'page0']));
        it('next', fn(pres, pres.searchByPage, [n, {}, 'initial', 'next', 1, 'next0']));
        it('next verify', fn(pres, pres.searchByPage, [n, {}, 'next0', 'self', 1, null]));
    });

    describe('prescription next transverse', function () {
        _.range(0, lastPage + 1).forEach(function (pageNo) {
            var pt = 'page' + pageNo;
            it(pt + ' link first', fn(pres, pres.searchByPage, [n, {}, pt, 'first', 0, null]));
            it(pt + ' link last', fn(pres, pres.searchByPage, [n, {}, pt, 'last', lastPage, null]));
            it(pt + ' link self', fn(pres, pres.searchByPage, [n, {}, pt, 'self', pageNo, null]));
            if (pageNo > 0) {
                it(pt + ' link prev', fn(pres, pres.searchByPage, [n, {}, pt, 'prev', pageNo - 1, null]));
            }
            if (pageNo < lastPage) {
                it(pt + ' link next', fn(pres, pres.searchByPage, [n, {}, pt, 'next', pageNo + 1, 'page' + (pageNo + 1)]));
            }
        });
    });
